...
export function AdminShell({ children, activePage = "nodes" }: { children: ReactNode; activePage?: "nodes" | "ping-targets" | "theme" | "settings" }) {
  ...
  return <div className="admin-shell"><header className="admin-header"><div className="admin-header-inner"><a href="/" onClick={(e) => { if (e.button === 0 && !e.metaKey && !e.ctrlKey) { e.preventDefault(); navigate("/"); } }} className="admin-brand"><strong>Monitor</strong><span>后台</span></a><div className="admin-header-actions"><button className="admin-header-button" type="button" aria-label="切换主题" onClick={() => theme.setTheme(theme.selected === "light" ? "dark" : theme.selected === "dark" ? "system" : "light")}><ThemeIcon size={18} aria-hidden="true" />主题</button><button className="admin-header-button" type="button" onClick={signOut}><LogOut size={18} aria-hidden="true" />退出</button></div></div></header><div className="admin-layout"><aside className="admin-sidebar" aria-label="后台导航"><NavItem icon={<Users />} label="节点" active={activePage === "nodes"} onClick={() => navigate("/admin/nodes")} /><NavItem icon={<CircleHelp />} label="延迟监控" active={activePage === "ping-targets"} onClick={() => navigate("/admin/ping-targets")} /><NavItem icon={<Tag />} label="主题" active={activePage === "theme"} onClick={() => navigate("/admin/theme")} /><NavItem icon={<Settings />} label="设置" active={activePage === "settings"} onClick={() => navigate("/admin/settings")} /></aside><main className="admin-content">{children}{logoutError && <div className="admin-inline-error" role="alert">{logoutError}</div>}</main></div></div>;
}
